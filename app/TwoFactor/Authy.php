...
class Authy implements TwoFactor
{
    ...
    public function sendToken(User $user)
    {
        try {
            $response = $this->client->request(
                'GET', 'https://api.authy.com/protected/json/sms/'.$user->twoFactor->identifier.'?api_key=' . config('services.authy.secret')
            );
        } catch (Exception $e) {
            return $e;
        }
    ...
    public function validateToken(User $user, $token)
    {
        try {
            $response = $this->client->request(
                'GET', 'https://api.authy.com/protected/json/verify/' . $token . '/' . $user->twoFactor->identifier . '?api_key=' . config('services.authy.secret')
            );
        } catch(Exception $e) {
            return false;
        }
